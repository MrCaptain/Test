...
%%成功: true, 全部成功
%%      {false, Reason/错误名字列表}
handle_call({'send_sys_mail', [NameList, Title, Content, GoodsList]}, _From, State) ->
    Reply = lib_mail:send_sys_mail(NameList, Title, Content, GoodsList),
    {reply, Reply, State};
...
handle_info(clean_overdure_mail, State) ->
    Now = util:unixtime(),
    Time = Now rem 86400,
    if Time >= 2*3600 andalso Time =< 6*3600 -> %%在2点及6点之间
...
terminate(_Reason, _State) ->
    ...
    io:format("~s mod_mail terminated~n",[misc:time_format(now())]),
    ok.
