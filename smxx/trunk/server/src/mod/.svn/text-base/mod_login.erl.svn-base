...
login(start, [PlayerId, AccountId, _Accname,ResoltX, ResoltY], Socket) ->
    case lib_account:check_account(PlayerId, AccountId) of
        false ->
            {error, fail1};
        true ->
			check_duplicated_login(PlayerId, AccountId),
			case mod_player:start(PlayerId, AccountId, ResoltX, ResoltY, Socket) of 
				{ok, Pid} ->
					{ok, Pid};
				_Err -> 
