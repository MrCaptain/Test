...
init({SceneAgentProcessName, Worker_id}) ->
	...
	if Worker_id =:= 0 ->
	   ...
	   true -> 
		   catch misc:unregister({local, SceneAgentProcessName}) ,
...
send_to_same_screen(SceneId, X, Y, BinData,ExceptUId)->
	gen_server:cast(get_agent_pid(), {send_to_local_same_screen, SceneId, X, Y, BinData,ExceptUId}) .
...
send_to_matrix(SceneId, X, Y, BinData) ->
	gen_server:cast(get_agent_pid(), {send_to_local_scene, SceneId, X, Y, BinData,""}) .
...
send_to_matrix(SceneId, X, Y, BinData,ExceptUId) ->
	gen_server:cast(get_agent_pid(), {send_to_local_scene, SceneId, X, Y, BinData,ExceptUId}) .												 
