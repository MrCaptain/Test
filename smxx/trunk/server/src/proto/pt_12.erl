...
%%%%玩家进入某场景
write(12002, [PlayerList,MonList,DropList]) ->
   ...
   {ok, pt:pack(12002, <<PlayerLen:16,PlayerBin/binary,MonLen:16,MonBin/binary,DropLen:16,DataBin/binary>>)};
...
write(12021, [Code,RevScn,RevX,RevY]) ->
	{ok, pt:pack(12021, <<Code:8,RevScn:16,RevX:8,RevY:8>>)} ;

%%掉落被拾取后的广播
write(12023, [MonId,GoodsId,PosX,PosY]) ->
	{ok, pt:pack(12023, <<MonId:32,GoodsId:32,PosX:8,PosY:8>>)} ;

%%宠物改名广播
write(12024, [Uid, PetTempId, Name]) ->
	{NmLen,NmBin} = tool:pack_string(Name) ,
	{ok, pt:pack(12024, <<Uid:64, PetTempId:32, NmLen:16, NmBin/binary>>)};

%%宠物休息,参战广播
write(12025, [Uid, PetTempId, Status]) ->
	{ok, pt:pack(12025, <<Uid:64, PetTempId:32, Status:8>>)};

%%宠物幻化广播 
write(12026, [Uid, PetTempId]) ->
	{ok, pt:pack(12026, <<Uid:64, PetTempId:32>>)};
...
pack_drop(DropRcd,NowTime) ->
	...
	LeftTime = data_config:get_config(drop_last_time) - (NowTime - NewDropRcd#mon_drop_goods.drop_time) ,
