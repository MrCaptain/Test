...
%% 查看NPC商人出售的物品
read(15015, <<ShopId:8, PageNo:8>>) ->
    {ok, [ShopId, PageNo]};

%% 购买NPC商人出售的物品
read(15016, <<ShopId:8, GoodsTid:32, Num:16>>) ->
    {ok, [ShopId, GoodsTid, Num]};

%% 装备物品  
read(15017, <<GoodsId:64, PetId:64>>) ->
    {ok, [GoodsId, PetId]};

%% 卸下装备 
read(15018, <<GoodsId:64, PetId:64>>) ->
    {ok, [GoodsId, PetId]};
...
%% 宝石神炼
read(15032, <<GoodsTid:32, BindFirst:8, Num>>) ->
...
write(15017, [Res, GoodsId, PetId, OldGoodsId, OldGoodsCell]) ->
	{ok, pt:pack(15017, <<Res:8, GoodsId:64, PetId:64, OldGoodsId:64, OldGoodsCell:16>>)};

%%卸下装备
write(15018, [Res, GoodsId, PetId, Cell]) ->
    {ok, pt:pack(15018, <<Res:8, GoodsId:64, PetId:64, Cell:16>>)};
