...
upd_daily_task_fin_2_db([State,CountDetail,CycDetail,TriDetail,Id])->
	...
	?DB_MODULE:update(daily_task_finish, 
								 ...
								 [{uid, PlayerId},
								  {type, Type}]);
upd_daily_task_fin_2_db([State,CycDetail,TriDetail,Id])->
		{PlayerId,Type} = Id,
	?DB_MODULE:update(daily_task_finish, 
								 [{state, State},
								  {cycle_datil,util:term_to_string(CycDetail)},
								 ...
								 [{uid, PlayerId},
								  {type, Type}]); 
upd_daily_task_fin_2_db([State,TriDetail,Id])->
	{PlayerId,Type} = Id,
	?DB_MODULE:update(daily_task_finish, 
								 [{state, State},
								  {trigger_detail,util:term_to_string(TriDetail)}], 
								 [{uid, PlayerId},
								  {type, Type}]).
upd_daily_task_fin_total([Total,NewTriDetail,Id])-> 
	{PlayerId,Type} = Id,
	?DB_MODULE:update(daily_task_finish, 
								 [{total,Total},
								  {trigger_detail,NewTriDetail}], 
								 [{uid, PlayerId},
								  {type, Type}]).
upd_daily_task_fin_total_state([Total,NewTriDetail,Id])->  
	{PlayerId,Type} = Id,
	?DB_MODULE:update(daily_task_finish, 
								 [{total,Total}
								  ,{state, 0}
