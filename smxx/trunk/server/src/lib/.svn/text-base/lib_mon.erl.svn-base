...
refresh_monster_drops([DropRcd|LeftDropList],DropList,NowTime) ->
	...
	case NowTime - DropTime < data_config:get_config(drop_last_time) of
		true ->
	...
	case  is_record(MonLayoutRcd, temp_mon_layout) of
		true ->
			...
			case CurrentHp > 0 of     %% 怪物活着
				...
				false ->			%% 怪物死亡
					%% 处理怪物掉落
					handle_monster_drop(MonLayoutRcd) ,
...
%% 	?TRACE("============do_refresh_monster: ~w ~n",[Mon#temp_mon_layout.state]),
	if
