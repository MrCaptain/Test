        ...
        true  ->  
            case get_mount(Status#player.id) of
                ...
                Mount ->
                    ...
                    if Status#player.mount >= 1 andalso Status#player.mount =:= Mount#mount.fashion ->
                           Status;
                       Status#player.mount >= 1 ->
                           Status#player{mount =  Mount#mount.fashion};
                       true ->
                           Status
                    end
            end;
...
open_mount(Status) ->
    ...
    Status#player{ mount = NewMount#mount.fashion,
        ...
        Mount when is_record(Mount, mount) ->
            case lists:member(FashionId, Mount#mount.fashion_list) of
                ...
                true ->
                    ...
                    ets:insert(?ETS_MOUNT, NewMount),
                    db_agent_mount:update_mount_fashion(NewMount),
                    {true, Status}        %返回成功
            end;
        _Other ->
            {false, 0}
...
%%上座骑
get_on_mount(Status) ->
    if Status#player.mount =:= 0 ->
        case get_mount(Status#player.id) of
            [] -> {false, 0};
            Mount ->
                %%更新玩家移动的计算速度
                NewStatus = Status#player{mount = Mount#mount.fashion},
                {true, NewStatus}
        end;
    true ->
        {false, 0}
...
get_off_mount(Status) -> 
    %%更新玩家移动速度
    NewStatus = Status#player{mount = 0}, %%有座骑并不使用
    NewStatus.
...
upgrade_mount_star(Status) -> 
    case get_mount(Status#player.id) of
        [] -> {false, 0};
        Mount ->
            NewStar = max(0, min(10, Mount#mount.star + util:rand(-3, 3))),
            NewMount = Mount#mount{star = NewStar}, 
            ets:insert(?ETS_MOUNT, NewMount),
            db_agent_mount:update_mount_star(NewMount),
            {true, NewStar}
    end.
...
get_mount_info(Status)->
    case get_mount(Status#player.id) of
        ...
        Mount ->        %%座骑存在,转换为消息喜欢的格式
            ...
            Riding = if Status#player.mount >= 1 -> 1; true -> 0 end,
            [1, Mount#mount.level, Mount#mount.star, Mount#mount.exp, Mount#mount.fashion, Riding, SkillList, FashionList]
    end.
