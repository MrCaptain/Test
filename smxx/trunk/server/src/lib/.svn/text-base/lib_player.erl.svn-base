...
recount_player_equip_attr(PlayerStatus) ->
	...
	% 套装装备加成
	SuitReward = lib_equip:get_equip_suit_reward(PlayerStatus, EquipList),
	% 镀金加成
...
revive(Status, here) -> 
    ...
    NewBattleAttr = OldBattleAttr#battle_attr{hit_point = round(OldBattleAttr#battle_attr.hit_point_max * data_config:get_config(here_revive_hp)),
...
revive(Status, city) -> 
    OldBattleAttr = Status#player.battle_attr,
    NewBattleAttr = OldBattleAttr#battle_attr{hit_point = round(OldBattleAttr#battle_attr.hit_point_max * data_config:get_config(city_revive_hp)),
