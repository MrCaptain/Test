...
%% 获取装备铸造属性
get_equip_casting_attri(PlayerStatus, EquipList) ->
	[].
...
%% 获取全身强化奖励
get_equip_total_reward(EquipList) ->
	[].
...
%% exports
%% desc: 查询物品的附加属性
get_goods_attri(PS, GoodsId) when is_integer(GoodsId) ->
    case goods_util:get_goods(PS, GoodsId) of
        GoodsInfo when is_record(GoodsInfo, goods) ->
            get_goods_attri(PS, GoodsInfo);
        _ ->
            []
    end;
get_goods_attri(PS, GoodsInfo) -> ok.
%%     case has_attribute(GoodsInfo) of
%%         true ->  lib_attribute:get_casting_attri_list(PS, GoodsInfo#goods.id);
%%         false -> []
%%     end.
