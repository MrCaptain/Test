...
%%@spec  ETS操作函数 begin==========
get_city_scene() ->
	MS = ets:fun2ms(fun(S) when S#temp_scene.mode =< 3 ->  S#temp_scene.sid end) ,
...
check_eneter(SceneId,Status,PosX,PosY) ->
	case get_scene_player(Status#player.id) of
...
enter_scene(Status) ->
	...
	%%2.0 获取场景上的怪物信息
	PlayerList = get_matrix_players(Status#player.scene,Status#player.battle_attr#battle_attr.x,Status#player.battle_attr#battle_attr.y,Status#player.resolut_x,Status#player.resolut_y) ,
	MonList = lib_mon:get_screen_monsters(Status#player.battle_attr#battle_attr.x,Status#player.battle_attr#battle_attr.y,Status#player.resolut_x,Status#player.resolut_y) ,
	?TRACE("===enter_scene:~p~n",[length(MonList)]) ,
	{ok,BinData} = pt_12:write(12002, [PlayerList,MonList]) ,
		...
		Player when is_record(Player,player) ->
			...
			case util:is_same_slice(OldX,OldY,DestX,DestY,Status#player.resolut_x,Status#player.resolut_y) of
				false ->
					...
					{ok,BinData} = pt_12:write(12002, [SlicePlayers,SliceMons]) ,
					lib_send:send_to_sid(Status#player.other#player_other.pid_send, BinData)  ;
...
%% 怪物掉落拾取
pick_drop(MonId, GoodsId, PosX, PosY) ->
	MonDropList = [ D || D <- lib_mon:get_monster_drops(),
						 D#mon_drop_goods.mon_id =:= MonId ,
						 D#mon_drop_goods.goods_id =:= GoodsId ,
						 D#mon_drop_goods.x =:= PosX,
						 D#mon_drop_goods.y =:= PosY  ] ,
	Fun = fun(D) ->
				  {D#mon_drop_goods.goods_id,D#mon_drop_goods.goods_num}
		  end ,
	lists:map(Fun, MonDropList) .
