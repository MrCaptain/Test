...
create_guild(PlayerId, Level, PlayerName, LastLoginTime, Gender, Career, ForceAtt, GuildName, GuildNotice) ->
    ...
    case db_agent_guild:insert_guild(Guild) of
        NewGuild ->
...
apply_join_guild(GuildId, PlayerId, Level, Name, Gender, Career, ForceAtt) ->
    ...
    NewApply = db_agent_guild:insert_apply(Apply),
    ets:insert(?ETS_GUILD_APPLY, NewApply),
    true.

%% 撤销加入帮派申请
cancel_join_apply(GuildId, PlayerId) ->
    delete_apply(GuildId, PlayerId),
    spawn(fun() -> db_agent_guild:delete_apply(GuildId, PlayerId) end),
    true.
...
accuse_chief(GuildId, PlayerId) ->
    ...
    if Guild =:= [] ->
       ...
       true ->
           ...
           db_agent_guild:update_accuse(GuildId, PlayerId, ExpireTime),
           db_agent_guild:update_vote(PlayerId, 1, ExpireTime),
           true
    end.
...
accuse_vote(GuildId, PlayerId, Operation) ->
    ...
    if Guild =:= [] ->
       ...
       true ->
           ...
           ets:insert(?ETS_GUILD_MEMBER, NewGuildMember),
           db_agent_guild:update_vote(PlayerId, Operation, NewGuild#guild.accuse_time),
           true
    end.
...
promote_member(GuildId, PlayerId, ?GUILD_ASSIST_CHIEF) ->
    ...
    if Guild =:= [] ->
       ...
       true ->
           ...
           ets:insert(?ETS_GUILD_MEMBER, NewGuildMember),
           db_agent_guild:update_position(PlayerId, ?GUILD_ASSIST_CHIEF),
...
promote_member(GuildId, PlayerId, ?GUILD_ELITE) ->
    ...
    if Guild =:= [] ->
       ...
       true ->
           ...
           ets:insert(?ETS_GUILD_MEMBER, NewGuildMember),
           db_agent_guild:update_position(PlayerId, ?GUILD_ELITE),
