						   ...
						   case ets:lookup(?ETS_TASK_DAILY_FINISH, {PlayerId,Type}) of
							  []->
								  ...
								  NewDailyFin = #daily_task_finish{
																   ...
																   total = 0,                     
																   trigger_time = date(),               
																   trigger_detail = {TaskDetail#task_detail.meanw_trigger,0}
																  },
								  ...
								  db_agent_task:new_daily_task_fin(
									[ PlayerId, Type, 0, 
									  ...
									  util:term_to_string({TaskDetail#task_detail.meanw_trigger,0}),
									  Now, 
									  0,
...
trigger_task(TaskId, PS) ->
	case tpl_task:get(TaskId) of
		...
		TD -> 
			...
			case Result of 
				true ->add_task_process(TD,PS,TaskId), 
...
check_daily_task_can_tri(TaskFin,Result)->
	{CanTri,NowTri} = TaskFin#daily_task_finish.trigger_detail,
	if CanTri-1 > NowTri ->
		  [{CanTri,NowTri+1}]++Result;
	    CanTri-1 == NowTri ->
			check_daily_task_can_cyc(TaskFin,[{CanTri,0}]++Result)
	end.
...
check_daily_task_can_cyc(TaskFin,Result)->
	...
	if CanCyc-1 > NowCyc ->
		  [?OUT_OF_MAX,{CanCyc,NowCyc+1}]++Result;
	    CanCyc-1 == NowCyc ->
...
check_daily_task_can_count(TaskFin,Result)->
	...
	case CanCount of
		...
		_-> 
			[?OUT_OF_MAX,{CanCount,NewCount}]++Result
	end.
...
upd_daily_task_fin_2_db_mem([TriDetail],TaskFin)->
	ets:insert(?ETS_TASK_DAILY_FINISH, TaskFin#daily_task_finish{  
																 trigger_detail = TriDetail 
																}),
		db_agent_task:upd_daily_task_fin_2_db([TriDetail, TaskFin#daily_task_finish.uid]).

%%任务限制判定(用于主线/支线任务)
check_trigger(TD, PS,Type) ->     
			case check_in_trigger(TD#tpl_task.tid, PS#player.id,TD#tpl_task.type) of
				%%该任务已经触发过了，不能接！
...
check_in_trigger(TaskId, Rid,Type)when Type=:=?MAIN_TASK orelse Type=:=?BRANCHE_TASK->
	 ets:lookup(?ETS_TASK_PROCESS, {Rid, TaskId}) =/= [];
check_in_trigger(_, Rid,Type)->
...
%%添加角色任务
add_task_process(TD,PStatus,TaskId)->
	ResMark =  [TD#tpl_task.target_type|TD#tpl_task.target_property],
	TaskType = TD#tpl_task.type,
	Date = date(),
	NewRoleTask =  #task_process{id={PStatus#player.id, TaskId},
								 uid=PStatus#player.id ,tid = TaskId, 
								 trigger_time =Date, state=0,
								 mark = ResMark,type=TaskType },
	Data = [PStatus#player.id,TaskId, 0,util:unixtime(),TaskType,util:term_to_string(ResMark)],
	db_agent_task:insert_task_process_data(Data),
	insert_pid(TaskType,0,TaskId),
...
upd_task_process(Type,Ps,ObjInfo) ->
	...
	case length(ResList) of
		...
		_-> 
			...
			{TaskFin,TaskUpd} = get_list_from_record(Rest), 
			%%通知客户端更新对应的任务
			call_back_task_process(TaskFin,30501,Ps),
			call_back_task_process(TaskUpd,30502,Ps) 
	end.
...
do_update(Type, ObjInfo,Task,Sum,PS) ->
	{ObjId,ObjNum,FinNum,NowNum} = convert_mark_data(Type,ObjInfo,Task#task_process.mark),
	%%进入完成任务处理逻辑
	if ObjNum +NowNum >= FinNum ->
		  check_auto_fin(Type,Task,Sum,PS,FinNum);
	   true -> %%进入未完成任务处理逻辑 
		   Result =  if is_integer(Sum) ->  
							#task_process_info{task_unfinsh =[{Task#task_process.tid,{ObjId, ObjNum +NowNum }}]};
						true->
							Sum#task_process_info{
												  task_unfinsh = lists:append
																   (Sum#task_process_info.task_unfinsh,[{Task#task_process.tid,{ObjId, ObjNum +NowNum }}])} 
					 end ,
...
do_upd_task_process(Task,Sum)->
	%%进入完成任务处理逻辑 
	Result = if is_integer(Sum) ->  
					#task_process_info{task_fin =[Task#task_process.tid]};
				true-> 
					Sum#task_process_info{
										  task_fin = lists:append(Sum#task_process_info.task_fin,[Task#task_process.tid])
										 } end,
...
%%在ets表中检测该任务状态
check_in_trigger(TaskId,PlayerId)->  
...
finish_task(TaskId,PS)-> 
	case tpl_task:get(TaskId) of
		Task when is_record(Task, tpl_task)->
			case check_in_trigger(Task#tpl_task.tid,PS#player.id) of
				true->do_finish(Task,PS#player.id,1,remote),  
			 ...
			 case do_player_get_goods(Task#tpl_task.goods_list,PS) of
				 NewPs when is_record(NewPs, player)-> 
					    {true,NewPs};
				 _->
					 ?ERROR_MSG("give task award to player fail ~n",[]),
					 {true,PS}
			 end;
				Other->
					Other 
...
do_player_get_goods(GoodList,PS)->  
	case parse_good_list(GoodList,PS) of
		AvailableList when length(AvailableList) >0 -> 
			%%这里到时候要多调用物品模块的一个接口
%% 		 goods_util:send_goods_and_money(AvailableList, PS); 
			true;
		_->
			skip
	end. 
...
do_normal_finish(Task,PlayerId) ->
	case get_one_daily_task_fin(PlayerId, Task#tpl_task.type) of
		[TD]->
		  db_agent_task:upd_daily_task_fin_total([TD#daily_task_finish.total+1,TD#daily_task_finish.uid]);
		_->skip
end.
...
refresh_active(PS) ->
	...
	call_auto_task_2_player(PS,TriId,30503).
...
auto_trgger_task(IdList,PS)-> 
	lists:foldl(fun(TaskId,TaskList)->
						case tpl_task:get(TaskId) of
							Task when is_record(Task, tpl_task)-> 
								if Task#tpl_task.start_npc =:=?TASK_AUTO_TRIG ->
									   add_task_process(Task,PS,TaskId),  
									   util:make_list(TaskList,TaskId); 
								   true ->
									   TaskList
								end;
							[]-> TaskList
	...
	case tpl_task:get(TaskId) of
		Task when is_record(Task, tpl_task)->  
				   case check_trigger_condition(Task, PS,Task#tpl_task.type) of
					   true ->
						   add_task_process(Task,PS,TaskId),  
						   case  pt_30:write(30506,[TaskId]) of
							   {ok,Data} ->
								   lib_send:send_to_sid(PS#player.other#player_other.pid_send, Data);
							   _->?WARNING_MSG("make proto 30506 error ~n",[])
						   end;
					   _->
						   skip 
				   end; 
		...
		Tpl when is_record(Tpl, tpl_task) ->
			case task_detail:get(Tpl#tpl_task.type) of
				Task when is_record(Task, task_detail) ->
					Task;
				_-> {false,?NO_TASK_IN_DETAIL}%任务不在任务模板子表
			end;
		_-> {false,?TASK_NOT_EXIT}%任务模板不存在
	end.
