...
%% handle_call(tolist, _From, State) ->
%%     % 优先处理WORLD节点
%%     R1 = case State#state.type of
%%         server  ->
%%             case rpc:call(node_world(), etimer, tolist, []) of
%%                 {badrpc, _} -> [];
%%                 L           -> L
%%             end;
%%         _       ->
%%             []
%%     end,
%%     Res0 = R1 ++ [X || X <- ets:tab2list(?GLOBAL_ETIMER)],
%%     Res1 = lists:keysort(#timer.id, Res0),
%%     {reply, Res1, State};
