         ...
         true  ->   %%命中
             case is_crit(Aer#battle_attr.crit_ratio, Der#battle_attr.tough_ratio) of
                 true  -> 
                     ...
                     TotalDamage = round_down(Damage  + SkillDamage + FmdDamage),
...
is_crit(AerCrit, DerTough) ->
    Crit = round_down(10000 * AerCrit / (AerCrit + DerTough * 10)),
...
is_hit(AerHit, DerDodge) ->
    DodgeRatio = round_down(10000  - (2500 + AerHit/(AerHit + DerDodge))),
    DodgeCmp = util:rand(1, 10000),
    DodgeRatio =< DodgeCmp.
