...
%%     						chat_script:call(11010,{<<"11">>},Socket), 
%% 		 					chat_script:call(11070,{1,<<"11">>},Socket), 
%% 	   	    			task_script:call(30003,0,Socket),
%%   					task_script:call(30004,0,Socket),
%%						task_script:call(30007,2,Socket),
%%   					task_script:call(30004,0,Socket),
%% 					task_script:call(30005,<<3:16,2:16,1:16,100:16>>,Socket),
%% 						task_script:call(30003,1,Socket),
  						task_script:call(30006,8,Socket),
%%  				    task_script:call(30006,2,Socket),
%%   					task_script:call(30003,2,Socket),
%%  					task_script:call(30003,4,Socket),
%%   					task_script:call(30003,5,Socket),
%% 					task_script:call(30003,6,Socket),
 							ok;
                        10007 ->
                            <<_Code:16>> = BinData,
                            ok;
                        _ -> 
								 %%邮件模块逻辑 
                   		 mail_script:handle_socket(Cmd,BinData),
						 %%聊天模块逻辑
 						 chat_script:handle_socket(Cmd, BinData)
%% 					,     no_action
                    end,
                    do_parse_packet(Socket, Pid);
                {fail, _} ->
