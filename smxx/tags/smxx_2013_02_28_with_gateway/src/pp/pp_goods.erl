...
handle(15003, PlayerStatus, Location) ->
	...
	goods_util:refresh_location(Res, PlayerStatus, Location),
	{ok, NewPlayerStatus};
...
handle(15007, PlayerStatus, [GoodsId, GoodsNum]) ->
	...
	lib_send:send_one(NewPlayerStatus#player.other#player_other.socket, BinData),
	goods_util:refresh_location(Res, NewPlayerStatus, ?LOCATION_BAG),
	{ok, no_save_online, NewPlayerStatus};
...
%% desc: 商城--购买商城普通区物品
handle(15014, PlayerStatus, [GoodsTid, GoodsNum, ShopType, ShopSubType]) ->
	{Res, NewPS} = mod_shop:handle_pay_goods(PlayerStatus, GoodsTid, GoodsNum, {ShopType, ShopSubType}),
	case Res =:= ?RESULT_OK of
		true ->  
			lib_player:refresh_client(NewPS, ?REFRESH_BAG),
			lib_task:event(shop_buy,GoodsTid, NewPS),
			mod_log:log_consume(?SHOP_N_EMPORIUM, GoodsTid, PlayerStatus, NewPS);
		_ ->
			skip
	end,
	lib_common:pack_and_send(NewPS, pt_15, 15014, [Res, GoodsTid, GoodsNum, ShopType, []]),
	{ok, no_save_online, NewPS}; 

%% desc: 商店--购买页--查看NPC出售列表
handle(15015, PlayerStatus, [NpcId, ShopSubType, PageNo]) ->
	[ShopList, TotalPage] = lib_shop:get_npc_equip_goods(PlayerStatus, NpcId, ShopSubType, PageNo),
	{ok, BinData} = pt_15:write(15015, [ShopList, TotalPage]),
	lib_send:send_one(PlayerStatus#player.other#player_other.socket, BinData);

%% desc: 商店--购买页--向NPC购买物品
handle(15016, PS, [NpcId, GoodsTid, Num]) ->
	[Res, NewPS] = mod_shop:handle_buy_npc_sells(PS, NpcId, GoodsTid, Num),
	lib_common:pack_and_send(NewPS, pt_15, 15016, Res),
	{ok, NewPS};

%% 装备物品（人物或宠物）
%% @para: PetId => 宠物唯一id（若是玩家穿装备，则为0；若是宠物穿装备，则为对应宠物的唯一id）
handle(15017, PS, [GoodsId, PetId]) ->
	[NewPS, Res, NewPartnerInfo, OldGoodsInfo] = gen_server:call(PS#player.other#player_other.pid_goods, {'equip', PS, GoodsId, PetId}, 500000),
...
handle(15018, PS, [GoodsId, PetId]) ->
	[NewPS, NewPartnerInfo, Res, GoodsInfo] = gen_server:call(PS#player.other#player_other.pid_goods, {'unequip', PS, [GoodsId, PetId]}, 500000),
	Cell =
...
%% 装备强化
handle(15019, NewPlayerStatus, [GoodsId, UnbindOnly, AutoBuy, Type]) ->
	[Res, NewPS, NewStren, PerfectDigree, Tstonenum, CostCoin, CostGold] = 
		gen_server:call(NewPlayerStatus#player.other#player_other.pid_goods, {'strengthen', NewPlayerStatus, GoodsId, UnbindOnly, AutoBuy, Type}),
...
handle(15023, PlayerStatus, [GoodsId | List]) ->
	...
	lib_send:send_one(NewPlayerStatus#player.other#player_other.socket, BinData),
	% 角色属性变更通知
	NewPstatus = mod_goods:refresh_client_aft_casting(backout, Res, GoodsId, NewPlayerStatus),
	lib_player:send_player_attribute3(NewPstatus),
	{ok, NewPstatus};

%% desc: 宝石合成
%% List = [StoneTypeId, UseRune, BindFirst, AutoBuy, StoneList]
handle(15024, PlayerStatus, List) ->
	[Res, NewPS, NewTid] = gen_server:call(PlayerStatus#player.other#player_other.pid_goods, {'compose', PlayerStatus, List}, ?DELAY_CALL),
	{ok, BinData} = pt_15:write(15024, [Res, NewTid]),
	lib_send:send_one(NewPS#player.other#player_other.socket, BinData),
	lib_player:send_player_attribute3(NewPS),
	{ok, NewPS};
...
%% 装备强化
handle(15029, NewPlayerStatus, [GoodsId, UnbindOnly]) ->
	[Res, NewPS, NewGildingLv, _CostCoin] = 
		gen_server:call(NewPlayerStatus#player.other#player_other.pid_goods, {'gilding', NewPlayerStatus, GoodsId, UnbindOnly}),
	NewPstatus = lib_player:calc_player_battle_attr(GoodsId, NewPS),
	lib_player:send_player_attribute3(NewPstatus),
...
%% 装备强化
%% handle(15031, NewPlayerStatus, [GoodsId, UnbindOnly, AutoBuy, Type]) ->
%% 	[Res, NewPS, NewStren, PerfectDigree, Tstonenum, CostCoin, CostGold] = 
%% 		gen_server:call(NewPlayerStatus#player.other#player_other.pid_goods, {'strengthen', NewPlayerStatus, GoodsId, UnbindOnly, AutoBuy, Type}),
%% 	NewPstatus = lib_player:calc_player_battle_attr(GoodsId, NewPS),
%% 	lib_player:send_player_attribute3(NewPstatus),
