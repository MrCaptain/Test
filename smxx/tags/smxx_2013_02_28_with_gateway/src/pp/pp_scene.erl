				...
				{ok,NewPlayer} ->
					...
					case Status#player.scene =/= NewSceneId andalso Status#player.scene > 999 of
						true ->
							...
							mod_scene_agent:send_to_matrix(NewPlayer#player.scene, NewPlayer#player.battle_attr#battle_attr.x, NewPlayer#player.battle_attr#battle_attr.y, LeaveBin) ;
						false ->
							skip
					end ,
					{ok, NewPlayer} ;
...
handle(12016, Status, [MonId, GoodsId,PosX,PosY]) ->
	[Code,GoodsList] = mod_scene:pick_drop(Status#player.scene,MonId, GoodsId,PosX,PosY) ,
	{ok,DropBin} = pt_12:write(12016, [Code]) ,
	lib_send:send_to_sid(Status#player.other#player_other.pid_send, DropBin) ,
	case Code of
		1 ->
			{NewStatus,_} = goods_util:send_goods_and_money(GoodsList, Status) ,
			{ok,NewStatus} ;
		_ ->
			ok
	end  ;
			...
			true ->
				case tpl_npc_layout:get(Status#player.scene div 100, NpcId) of
					NpcLayout when is_record(NpcLayout,temp_npc_layout) ->
						...
						case (DistX > 2 andalso DistY > 2) orelse (DistX < 1 andalso DistY < 1) of
							false ->
...
handle(12021, Status, [_SceneId]) ->
	...
	%% 1.0 向原场景或者副本发送离开消息
	{ok, BinData} = pt_12:write(12004, [NewStatus#player.id]),
	 mod_scene_agent:send_to_matrix(NewStatus#player.scene, NewStatus#player.battle_attr#battle_attr.x, NewStatus#player.battle_attr#battle_attr.y, BinData,NewStatus#player.id),
	
	%% 2.0 获取复活场景
	case lib_scene:get_scene(NewStatus#player.scene) of
		Scene when is_record(Scene,temp_scene) ->
			handle(12001, NewStatus, [Scene#temp_scene.revive_sid,Scene#temp_scene.revive_x,Scene#temp_scene.revive_y]) ;
		_ ->
			skip
	end ,
	
	%% 同步玩家状态到场景
	{ok, ReviveBinData} = pt_12:write(12003, [NewStatus]),
	mod_scene_agent:send_to_matrix(NewStatus#player.scene, NewStatus#player.battle_attr#battle_attr.x,NewStatus#player.battle_attr#battle_attr.y,ReviveBinData,NewStatus#player.id) ,
