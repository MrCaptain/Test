...
findAndModify(Collection, Table_name, Key) ->
	Id = emongo:findAndModify(tool:to_list(?MASTER_POOLID),tool:to_list(Collection), tool:to_list(Table_name), tool:to_list(Key)),
	case Id of 
		undefined -> 0;
		Num when Num < 100000000 ->
%% 			Id;
			ServerNum = config:get_server_num(),
			if ServerNum > 0 ->
				   NewId = (ServerNum * 100000000) + 1,
				   NewOverId = emongo:findAndModifyNewId(tool:to_list(?MASTER_POOLID),tool:to_list(Collection), tool:to_list(Table_name), tool:to_list(Key), NewId),
				   case NewOverId of
					   undefined -> 0;
					   _ -> NewOverId
				   end;
...
%% io:format("insert_1_/~p/~n",[Table_name]),
	...
	IdValue = findAndModify("auto_ids", Table_name, "id"),
...
fullKeyValue(Table_name,KeyValuesList) ->
	 ...
	 L = [F({Key, DefaultValue})||{Key, DefaultValue}<- FieldsDefaultValueList],
...
select_count(Table_name, Where_List) ->
	Data = emongo:count(tool:to_list(?MASTER_POOLID),tool:to_list(Table_name),db_mongoutil:make_where_opertion(Where_List)),
...
%%  io:format("L__/~p/~p/~p/~p/ ~n",[WhereOpertion,FieldOpertion, ?MASTER_POOLID, L]),		
