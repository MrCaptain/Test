...
handle_cast({send_to_same_screen, SceneId, X, Y, BinData,ExceptUId}, State) ->
	WorkerId = random:uniform(?SCENE_WORKER_NUMBER),
	SceneAgentWorkerName = misc:create_process_name(scene_agent_p, [WorkerId]),
	gen_server:cast(tool:to_atom(SceneAgentWorkerName), {send_to_local_same_screen, SceneId, X, Y, BinData,ExceptUId}),
	{noreply, State};
...
handle_cast({send_to_local_same_screen, SceneId, X, Y, BinData,ExceptUId}, State) ->
...
handle_cast({broadcast_move, SceneId, OldPosXY, NewPosXY, ResolutXY, LeaveBinData, EnterBinData},State) ->
	WorkerId = random:uniform(?SCENE_WORKER_NUMBER),
	SceneAgentWorkerName = misc:create_process_name(scene_agent_p, [WorkerId]),
	gen_server:cast(tool:to_atom(SceneAgentWorkerName), {broadcast_move_worker, SceneId,OldPosXY,NewPosXY, ResolutXY, LeaveBinData, EnterBinData}),
	{noreply, State};
%% 新版人物位置同步
handle_cast({broadcast_move_worker, SceneId,OldPosXY,NewPosXY,ResolutXY, LeaveBinData, EnterBinData}, State) ->
...
send_to_matrix(SceneId, X, Y, BinData,ExceptUId) ->
	[gen_server:cast(Pid, {send_to_scene, SceneId, X, Y, BinData,ExceptUId}) || Pid <- misc:pg2_get_members(scene_agent)].
