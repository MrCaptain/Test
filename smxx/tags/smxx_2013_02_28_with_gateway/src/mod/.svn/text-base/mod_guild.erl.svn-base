...
get_guild_pid(GuildId) ->
    ...
    case misc:whereis_name({global, MainProcName}) of
            Pid when is_pid(Pid) ->
                case misc:is_process_alive(Pid) of
                    true ->
                        if GuildId =:= 0 ->
...
init([WorkerId]) ->
    ...
    ProcessName = misc:create_process_name(guild_p, [WorkerId]),
    case misc:register(unique, ProcessName, self()) of  %%多节点的情况下， 仅在一个节点启用帮派处理进程
        yes ->
            if WorkerId =:= 0 ->    
                   ...
                   ets:new(?ETS_GUILD_APPLY, [{keypos,#guild_apply.id}, named_table, public, set]),       %%帮派申请
