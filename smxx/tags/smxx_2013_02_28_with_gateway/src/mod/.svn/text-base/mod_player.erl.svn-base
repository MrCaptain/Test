...
%%          {stop, Reason}
%% --------------------------------------------------------------------
init([PlayerId, AccountId, ResoltX,ResoltY, Socket]) ->
...
handle_cast({add_goods_buff, BuffId}, Status) ->
    NewStatus = buff_util:add_goods_buff(Status, BuffId),
    {noreply, NewStatus};

%%刷新Buff, 定时器调用或强制刷新
%%刷新Buff会自动把效的Buff移除
handle_cast(refresh_goods_buff, Status) ->
    ...
    if Num > 0 ->
       ...
       true ->
           ...
           spawn(fun()-> db_log_agent:insert_kick_off_log(Status#player.id, Status#player.nick, 7, Now, 
                                                          Status#player.scene, Status#player.battle_attr#battle_attr.x, Status#player.battle_attr#battle_attr.y, NewTimeListStr) end),
...
load_for_runtime_data(Player, ResoltX, ResoltY, Socket, LastLoginTime) ->
    ...
    [WeaponStrenLv, ArmorStrenLv, FashionStrenLv, WaponAccStrenLv, WingStrenLv, _] = lib_equip:get_equip_strenlv(GoodsStatus#goods_status.equip_current, PlayerTmp, []),
...
test() ->
	[Status|_] = ets:tab2list(ets_online_scene) ,
...
set_donttalk(PlayerId, {BeginTime, DurationSeconds}) ->
      gen_server:cast({global, misc:player_process_name(PlayerId)}, {set_donttalk, BeginTime, DurationSeconds}).
