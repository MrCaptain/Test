...
init([PlayerId, CellNum, GoodsEtsId]) ->
    ...
    InitEquip = [?DEFAULT_T_WEAPON, ?DEFAULT_T_ARMOR, ?DEFAULT_T_FASHION, ?DEFAULT_T_WEAPONACCESSORIES, ?DEFAULT_T_WINGS, ?DEFAULT_T_MOUNT],
...
%%                 [SuitNum, AttriList] = case is_record(GoodsInfo, goods) of
%%                                            true when GoodsInfo#goods.suit_id > 0 -> 
%%                                                [1, lib_equip:get_goods_attri(PS, GoodsInfo)];
%%                                            true ->
%%                                                [0, lib_equip:get_goods_attri(PS, GoodsInfo)];
%%                                            false -> 
%%                                                [0, []]
%%                                        end,
    [SuitNum, AttriList] = [0, []],
            ...
            Online ->                
                case goods_util:get_goods(Online, GoodsId) of
                    GoodsInfo when is_record(GoodsInfo, goods) andalso GoodsInfo#goods.uid =:= RoleId ->
                        AttributeList = lib_equip:get_goods_attri(Online, GoodsInfo),
                        SuitNum = 0,
...
handle_cast({'list', PlayerStatus, Location}, GoodsStatus) ->
    ...
    {ok, BinData} = pt_15:write(15002, [Location, CellNum, PlayerStatus#player.bcoin, PlayerStatus#player.coin, PlayerStatus#player.bgold, PlayerStatus#player.gold, TotalList]),
...
handle_cast({'drag', PlayerStatus, GoodsId, _OldCell, NewCell}, GoodsStatus) ->
    [Result, CellId1, TId1, CellId2, TId2, NewGS, OldCell, Location] =
        case goods_util:check_drag(PlayerStatus, GoodsStatus, GoodsId, NewCell) of
            {fail, Res} ->
                [Res, 0, 0, 0, 0, GoodsStatus, 0, none];
            {ok, GoodsInfo} when GoodsInfo#goods.cell =:= NewCell ->
                [?RESULT_OK, 0, 0, 0, 0, GoodsStatus, 0, none];
            {ok, GoodsInfo} ->
                case (catch goods_util:drag_goods(PlayerStatus, GoodsStatus, GoodsInfo, NewCell)) of
                    ...
                    Error ->
                        ...
                        [?RESULT_FAIL, 0, 0, 0, 0, GoodsStatus, 0, none]
                end
        end,
...
%% 赠送物品
handle_cast({'give_goods', GoodsTypeId, GoodsNum}, GoodsStatus) ->
    case lib_goods:give_goods([{GoodsTypeId, GoodsNum}], GoodsStatus) of
        NewGS when is_record(NewGS, goods_status) ->
            lib_player:refresh_client(NewGS#goods_status.uid, 2),
            {noreply, NewGS};
        _Error ->
...
%% desc: 给玩家赠送物品
handle_cast({'give_goods', BindState, Location, GoodsList}, GoodsStatus) ->
    case lib_goods:give_goods(BindState, Location, GoodsList, GoodsStatus) of
        NewGS when is_record(NewGS, goods_status) ->
            {noreply, NewGS};
        _Error ->
            ?ERROR_MSG("mod_goods give_goods:~p", [_Error]),
...
handle_cast({'throw', PlayerStatus, GoodsId, GoodsNum}, GoodsStatus) ->
    {Result, NewGS}=  case goods_util:check_throw(PlayerStatus, GoodsStatus, GoodsId, GoodsNum) of
					        ...
					        {ok, GoodsInfo} ->
					            case (catch lib_goods:delete_one(PlayerStatus, GoodsStatus, GoodsInfo, GoodsNum)) of
					                {ok, NewStatus, _} ->
					                    {?RESULT_OK, NewStatus};
					                Error ->
...
handle_cast({'GET_CASTING_INFO', PS, [GoodsId, Type]}, GS) ->
    [Info, Snum, AttrList] =
        case lib_equip:is_equip(PS, GoodsId) of
            {true, GoodsInfo} -> 
               casting_util:calc_upgrage_info(PS, GoodsInfo);
            false ->               [{}, 0, []]
        end,
...
handle_call({'extend', PlayerStatus, Location}, _From, GoodsStatus) ->
    ...
    case goods_util:check_extend(PlayerStatus, Location, Type) of
        ...
        {ok, Cost, MaxCell, Cells} ->
            case (catch goods_util:extend_bag(PlayerStatus, Cost, Type, MaxCell, Cells, Location, GoodsStatus)) of
    ...
    case goods_util:check_equip(PlayerStatus, GoodsId, PetId, Location) of
        ...
        {ok, GoodsInfo} ->
            case (catch goods_util:equip_goods(PlayerStatus, GoodsStatus, GoodsInfo, PetId, Location)) of
                {ok, NewPlayerStatus, NewPartnerInfo, NewStatus, OldGoodsInfo} ->
                	NewPlayerStatus1 =
                    ...
                    {reply, [NewPlayerStatus1, ?RESULT_OK, NewPartnerInfo, OldGoodsInfo], NewStatus};
                Error ->
    ...
    case goods_util:check_unequip(PlayerStatus, GoodsStatus, [GoodsId, PetId, Location]) of
        ...
        {ok, GoodsInfo} ->
            case (catch goods_util:unequip_goods(PlayerStatus, GoodsStatus, GoodsInfo, PetId, Location)) of
                {ok, NewPlayerStatus, NewPartnerInfo, NewStatus, NewGoodsInfo} ->
                     {reply, [NewPlayerStatus, NewPartnerInfo, ?RESULT_OK, NewGoodsInfo], NewStatus};
                 Error ->
...
%% 15023 宝石拆除
handle_call({'backout', PlayerStatus, InputList}, _From, GoodsStatus) ->
    case lib_check_casting:check_backout(PlayerStatus, GoodsStatus, InputList) of
        {fail, Res} ->
            {reply, [Res, PlayerStatus], GoodsStatus};
        {ok, GoodsInfo, Rune, StoneTypeIdList, CostList} ->
			case handle_casting(equip_backout, PlayerStatus, GoodsStatus, [GoodsInfo, Rune, StoneTypeIdList, CostList]) of
				[Res, NewStatus, NewPS] ->
...
%% 15024 宝石合成
handle_call({'compose', PlayerStatus, List}, _From, GoodsStatus) ->
    case lib_check_casting:check_compose(PlayerStatus, GoodsStatus, List) of
        {fail, Res} ->
            {reply, [Res, PlayerStatus, 0], GoodsStatus};
        {ok, CheckedList} ->
			case handle_casting(equip_compose, PlayerStatus, GoodsStatus, [CheckedList]) of
				[Res, NewStatus, NewPS, NewTid] ->
					{reply, [Res, NewPS, NewTid], NewStatus};
				Error ->
					?ERROR_MSG("mod_goods equip_compose:~p", [Error]),
					{reply, [?RESULT_FAIL, PlayerStatus, 0], GoodsStatus}
			end
	end;
...
handle_call({'get_goods_num', GoodsTid, BindState}, _From, GoodsStatus) ->
    case goods_util:get_type_goods_list(GoodsStatus#goods_status.uid, GoodsTid, BindState, ?LOCATION_BAG) of
...
%% desc: 删除背包中某一种物品指定个数
handle_call({'delete_bag_goods', PS, GoodsTid, BindState, Num}, _From, GoodsStatus) ->
    case goods_util:get_type_goods_list(GoodsStatus#goods_status.uid, GoodsTid, BindState, ?LOCATION_BAG) of
        List when is_list(List) ->
            ...
            case Nums >= Num of
                ...
                true ->
                    {ok, NewStatus} = lib_goods:delete_more({PS, GoodsStatus}, List, Num),
...
handle_call({'delete_one', PS, GoodsId, GoodsNum}, _From, GoodsStatus) ->
    ...
    if
        ...
        true ->
            case (catch lib_goods:delete_one(PS, GoodsStatus, GoodsInfo, GoodsNum)) of
                {ok, NewStatus, NewNum} ->
...
%% desc: 删除多个格子的同类型物品
handle_call({'delete_more', PS, GoodsTypeId, GoodsNum}, _From, GoodsStatus) ->
    GoodsList = goods_util:get_type_goods_list(GoodsStatus#goods_status.uid, GoodsTypeId, ?LOCATION_BAG),
    ...
    if 
        ...
        TotalNum < GoodsNum ->
            {reply, 3, GoodsStatus};
        true ->
            case (catch lib_goods:delete_more({PS, GoodsStatus}, GoodsList, GoodsNum)) of
                {ok, NewStatus} ->
                     lib_player:refresh_client(GoodsStatus#goods_status.uid, 2),
...
%% 赠送物品
handle_call({'give_goods', GoodsTypeId, GoodsNum}, _From, GoodsStatus) ->
    case lib_goods:give_goods([{GoodsTypeId, GoodsNum}], GoodsStatus) of
        NewGS when is_record(NewGS, goods_status) ->
            lib_player:refresh_client(NewGS#goods_status.uid, ?REFRESH_BAG),
            {reply, ok, NewGS};
        _Error ->
            ?ERROR_MSG("mod_goods give_goods:~p", [_Error]),
            {reply, _Error, GoodsStatus}
    end;
handle_call({'give_goods', BindState, Location, GoodsList}, _From, GoodsStatus) ->
    case lib_goods:give_goods(BindState, Location, GoodsList, GoodsStatus) of
        NewGS when is_record(NewGS, goods_status) ->
            {reply, ok, NewGS};
        _Error ->
...
%% desc: 给玩家赠送物品和钱
handle_call({'give_goods_and_money', PS, BindState, Location, GoodsList}, _From, GoodsStatus) ->
    case lib_goods:give_goods_and_money(BindState, Location, GoodsList, PS, GoodsStatus) of
...
%% 宝石合成
handle_casting(equip_compose, PlayerStatus, GoodsStatus, [CheckedList]) ->
	case ( catch casting_util:compose(PlayerStatus, GoodsStatus, CheckedList) ) of
		{ok, NewPS, NewStatus, NewTid} ->   % 宝石合成成功
			[?RESULT_OK, NewStatus, NewPS, NewTid];
        {fail, NewPS, NewStatus, NewTid} ->   % 宝石合成失败
			[?RESULT_FAIL, NewStatus, NewPS, NewTid];
		Error -> 
            Error
    end;
