...
%%获取socket组名称
get_socket_group_name(Accid) ->
	tool:to_atom(lists:concat(["sg_",Accid])).
...
init([GroupName,Socket,Player_id,Player_pid]) ->
	...
	%%?DEBUG("_______________________________________________________________INIT:~p:~p",[Socket,GroupName]),
    {ok, State}.
