...
-define(COLOR_PURPLE,   2).    % 紫色
-define(COLOR_ORANGE,   3).    % 橙色
-define(COLOR_YELLOW,   4).    % 黄色(注：没有黄色的物品，但有黄色的武将)
-define(COLOR_GREEN,    5).    % 绿色
...
-define(BIND_ALREADY, 3).      % 可绑定并且已绑定
%% 交易状态
...
-define(CELL_NUM_MAX_BAG, 36 * 3).   % 玩家背包最大格子数（包括未开启的格子）
...
-record(goods_status, {
        ...
        equip_current = [0,0,0,0],      % 当前玩家主角的装备类型ID - [武器, 衣服, 翅膀, 时装]
...
%% desc: 强化完美度最大值
-define(MAX_STREN_DEGREE, 100).
-define(LEVEL_STREN_DEGREE, 10).
