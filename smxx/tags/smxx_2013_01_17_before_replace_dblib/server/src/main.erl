...
-export([gateway_start/0, gateway_stop/0, server_start/0, server_stop/0, server_remove/0, info/0, init_db/1]).
...
init_mysql_admin(App) ->
    ...
    mysql:start_link(?DB_SERVER_ADMIN,?DB_POOL, Host, Port, User, Password, DB,  fun(_, _, _, _) -> ok end, Encode),
    mysql:connect(?DB_SERVER_ADMIN,?DB_POOL, Host, Port, User, Password, DB, Encode, true),
