...
start(Options) ->
    start(Options, fun mysql:start/9, false).
...
start_link(Options) ->
    start(Options, fun mysql:start_link/9, true).
...
start(Options, Fun, LinkConnections) -> 
    ...
    Fun(mysql_dispatcher, PoolId1, Hostname, Port, Username, Password, Database, LogFun,
	Encoding),
...
q2(Statement, Options) ->
    mysql:fetch(mysql_dispatcher, get_pool_id(Options), Statement, get_timeout(Options)).
