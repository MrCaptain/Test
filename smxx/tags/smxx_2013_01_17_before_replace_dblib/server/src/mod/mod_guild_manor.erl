...
-record(state, {manor_scene_id = 0,		%%联盟领地唯一id
				scene_id = 0,			%%场景原始id
				guild_id = 0,			%%联盟id
...
%% 	end.
%% %%玩家下线了，列表对应的数据设为offline
%% player_logout_manor(Status) ->
%% 	#player{id = PlayerId,
%% 			scn = UniqueSceneId,
%% 			unid = GuildId} = Status,
%% 	case lib_guild_manor:is_guild_manor_scene(UniqueSceneId, GuildId) of
%% 		true ->
%% 			PidScene = Status#player.other#player_other.pid_scene,
%% 			logout_manor_role_list(PidScene, PlayerId);
%% 		false ->
%% 			no_action
%% 	end.
%% 
%% logout_manor_role_list(PidManor, PlayerId) ->
%% 	case misc:is_process_alive(PidManor) of
%% 		false ->
%% 			false;
%% 		true ->
%% 			PidManor ! {logout, PlayerId}
%% 	end.
%% 
...
%% 	lib_guild_manor:check_use_guild_token(Status).
%% 
%% %% --------------------------------------------------------------------
%% %% Function: init/1
...
%% 	misc:register(global, ProcessName, self()), %% 多节点的情况下， 仅启用一个对应的联盟进程
%% 	case PlayerId of
%% 		0 ->
%% 			ManorRoleList = [];
%% 		_ ->
...
%% 	%%广播
%% 	ConTent = io_lib:format("<font color='#FEDB4F;'>[~s]</font>穿过了时空的裂缝出现在<font color='#FEDB4F;'>[~s]</font>的联盟领地上，联盟成员们请速去支援族长。",
%% 							[MonName, GuildName]),
...
%% 			{X, Y} = lists:nth(Num, 2), %%随机产生一对坐标
%% 			[500, X, Y];
%% 		_NewSceneId when SceneId > 2 ->
...
%% %% 40026  获取联盟高级技能信息
%% %% -----------------------------------------------------------------
%% get_guild_skilltoken(GuildId) ->
%% 	try 
%% 		case gen_server:call(mod_guild:get_mod_guild_pid(), 
%% 							 {apply_call, lib_guild_weal, get_guild_skilltoken, 
%% 							  [GuildId]})	of
%% 			 error -> 
%% 				 %% ?DEBUG("40001 create_guild error",[]),
%% 				 [0, []];
...
%% 									   Status#player.upst, GuildId, Type]})	of
%% 					error -> 
%% 						%% ?DEBUG("40001 create_guild error",[]),
%% 						[0];
%% 					Data -> 
