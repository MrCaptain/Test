                                          ...
                                          receive
                                                 {inet_async, Socket, Ref1, {ok, Binary}} when Cmd =:= 10010->
                                                  <<AccId:32, PlayerId:64, Bin/binary>> = Binary,
                                                      ...
                                                      ?TRACE("~s Create role for: Accid:~p, AccName: ~p, PlayerId:~p~n",
                                                          [misc:time_format(now()), AccId, Accname, PlayerId]),
                                              ...
                                              Other ->
                                                  ?TRACE("create account failed:~p~n",[Other]),
                                                  gen_tcp:close(Socket),
                                                  error_60
                                          end;
                                      false ->
                                          ?TRACE("10010: create account failed for guest~n"),
                                          error_70
                                  end;                       
                              %%用户断开连接或出错
                              _Other ->
                                  ?TRACE("create account failed: ~p~n",[_Other]),
                                  gen_tcp:close(Socket),
                                  error_80
                          end
                       catch
                           _:_ ->
                               ?TRACE("login recving date error~n"),
                               gen_tcp:close(Socket),
                               error_90
                       end;
                 _Reason ->
                       ?TRACE("connect server ip/port failed: ~p~n", [_Reason]),
                       error_100
             end;
        _Reason1 ->    
...
%%选择角色进入
handle(enter_player, {PlayerId}, Socket) ->
    gen_tcp:send(Socket, pack(10004, <<9999:16,PlayerId:64>>)),    
    %gen_tcp:send(Socket, pack(12002, <<>>)),    
    %erlang:send_after(random:uniform(?ACTION_INTERVAL * 2)+1000, self(), {event, action_random, PlayerId, Socket}),    
    ok;
